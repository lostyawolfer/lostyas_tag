...
execute if score taggers-percent server matches ..19 if score game server matches 1..6 unless score game server matches 4 unless score tag.random_counter server matches -30.. unless score restart server matches 0..400 unless score kill_timer server matches -1..0 if entity @a[tag=!tagger, gamemode=adventure] if score playercount server matches 2.. run function tag:-/tag_randomize_add
...
execute as @a[tag=!tagger, gamemode=adventure, tag=!safezone, tag=!special] at @s if entity @a[tag=tagger, gamemode=adventure, tag=!safezone, distance=..24] unless score game server matches 4 run scoreboard players add @s tagger_heartbeat 1
