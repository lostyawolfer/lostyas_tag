...
execute if score @s anticheat matches 7.. run tellraw @a [{"text":"! [","color":"dark_red"},{"text":"TAGGUARD","color":"gold"},{"text":"]: ","color":"dark_red"},{"selector":"@s","color":"yellow"},{"text":" is flying or blinking!","color":"red"}]
#execute if score @s anticheat matches 7.. run tag @s add dead
execute if score @s anticheat matches 7.. run scoreboard players reset @s anticheat