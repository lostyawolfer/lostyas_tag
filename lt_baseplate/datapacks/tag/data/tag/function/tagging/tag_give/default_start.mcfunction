tellraw @a[scores = {logging = 1}] ["! log: ", {"selector": "@a[tag = hit_detect.giver]"}, " tagged ", {"selector": "@a[tag = hit_detect.taker]"}, ", which started the game"]

tag @a remove special
tag @a remove tagger
scoreboard players set game server 1
